...
async function httpNetworkFetch (
  ...
  function dispatch ({ body }) {
    ...
    // Resolve origin per fetch spec - https://fetch.spec.whatwg.org/#resolve-an-origin
    // If origin's host's public suffix is "localhost" or "localhost.",
    // then return « ::1, 127.0.0.1 ».
    let resolvedOrigin = url.origin
    let dispatchHeaders = request.headersList.entries

    if (isLocalhostDomain(url.hostname)) {
      // Create new URL with resolved localhost address
      // Prefer IPv4 for better compatibility
      const resolvedUrl = new URL(url)
      resolvedUrl.hostname = '127.0.0.1'
      resolvedOrigin = resolvedUrl.origin

      // Preserve original hostname in Host header for SNI/virtual hosting
      // Clone headers object and set Host header to original hostname
      dispatchHeaders = { ...request.headersList.entries, host: url.host }
    }
