...
/**
 * Check if hostname is a localhost subdomain per RFC 6761 and Fetch spec
...
function isLocalhostDomain (hostname) {
  ...
  // Check for exact localhost matches (with or without trailing dot)
  if (normalizedHostname === 'localhost' || normalizedHostname === 'localhost.') {
    return true
  }

  // Check for localhost subdomains (*.localhost or *.localhost.)
  if (normalizedHostname.endsWith('.localhost') || normalizedHostname.endsWith('.localhost.')) {
    return true
  }
